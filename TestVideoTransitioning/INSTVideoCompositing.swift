...
//      print(asyncVideoCompositionRequest.compositionTime.seconds)
  ...
  private func render(frameOne: CVPixelBuffer, with frameTwo: CVPixelBuffer, to destination: CVPixelBuffer, transform: CGAffineTransform, frameTwoProgress: Int) {
    ...
    if progress1 < 1 {
      xOffset1 = Curve.exponential.easeInOut(progress1) * CGFloat(width) //easeInOutQuad(percentComplete: progress1, elapsedTimeMs: progress1, startValue:0, endValue: CGFloat(width), totalDuration: 1.0)
    } else {
    ...
    if progress2 == 0 {
      ...
      xOffset2 = Curve.quintic.easeOut(progress2) * CGFloat(width) //easeInOutQuad(percentComplete: progress2, elapsedTimeMs: progress2, startValue:0, endValue: CGFloat(width), totalDuration: 1.0)
    } else {
...
//    let baseImg = CIImage(cvPixelBuffer: buffer)
//    var resultImg = baseImg.cropped(to: CGRect(x: 0, y: 0, width: width, height: height))
//    resultImg = resultImg.transformed(by: preferredTransform)
//
